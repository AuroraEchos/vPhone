...
> 截图是唯一页面观测输入；未来的多模态模型结合用户任务和当前截图作语义决策。

真实 App 的控件树可能缺失或不可靠，而现代多模态模型可以直接处理页面截图。L3 因此只建立一条清晰的观测路径：从 L1 获取当前完整截图，形成不可变的页面观测，交给未来的 L4。它不预先提取文字、图标、控件属性或点击坐标，也不读取 App 对外暴露的元数据。

```text
L1 截图 → L3 PageObservation → L4 多模态决策（后续）
                                      ↓ 具体动作
                                    L2 执行
                                      ↓
                            再次截图并评估实际效果
...
当前代码没有 L4，因而尚不能自主完成自然语言任务。开发者或测试代码需要根据截图选择动作，并在执行后重新观测。
...
L3 不派发动作，也不根据截图内容判断任务成功。L2 的动作结果仅说明设备原语调用情况；点击、滑动、输入后应重新调用 `observe()`，由未来的 L4 比较新旧页面并判断实际效果。对于付款、删除或发送等不可逆操作，决策和确认策略属于 L4。
...
- 与用户一起定义 L4 如何把截图和任务提供给多模态模型，以及模型可选择的受约束动作工具。
