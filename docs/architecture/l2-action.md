...
- `ScreenshotError`：通常出现在重新观测阶段，不是 L2 派发本身产生的成功判定。

更细的错误定义与 ADB 映射见 [L1 设备层设计](l1-device.md#5-错误超时与资源限制)。

## 5. 与 L3 / L4 的协作约定

项目已转向纯视觉感知：截图是页面状态的唯一观测输入；L3 只交付当前截图，不预先运行 OCR 或生成元素候选。L4 的多模态模型结合用户任务与截图提出具体动作，L4 本地控制器验证后派发。L2 不承担感知或语义决策，只接收最终确定的具体动作。实现边界见 [L3 感知层](l3-perception.md)与 [L4 决策层](l4-planner.md)。
...
坐标来自过去的截图，天然可能过期。滑动、旋转、弹窗后尤其需要重新截图定位。只有 L4 能根据任务语义判断“是否值得再试”，L2 不知道某次点击是否具有付款、删除、发送等不可逆后果。
...
- 若未来需要“点击视觉目标”，应保持定位与执行分离：由 L4 结合截图确定具体坐标，再交给 L2 的坐标动作；不要让 L2 隐式查找目标。
